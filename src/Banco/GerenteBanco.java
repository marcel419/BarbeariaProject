...
public class GerenteBanco extends Conecta {
	...
	public void CadastrarGerente(Gerente gerente) throws SQLException, Exception {
	...
	public void DeletarGerente(Gerente gerente) throws SQLException, Exception {
	...
	public void AtualizarGerente(Gerente gerente) throws SQLException, Exception {
		
		String query = "UPDATE gerente SET nome = ?, cpf = ?, email = ?, endereco = ?, telefone = ?, dataNascimento = ? WHERE id = ?";
		 ps = super.conectar().prepareStatement(query);
			 ps.setString(1, gerente.getNome()); 
			 ps.setString(2, gerente.getCpf()); 
			 ps.setString(3, gerente.getEmail());
